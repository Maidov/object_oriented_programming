...
Figure::Figure(point &center, int n){    // INITIALIZATION WITH DOT AND CENTER DOT
    this->_size = n;
    arr = new point[n];
    
    // std::cout << "Created Fig with " << this->_size << " dots\n";
    // std::cout << "With center in (" << this->_center.x << ", " << this->_center.y << ") dot\n";
}


Figure::Figure(const std::initializer_list<point> &dots, int n){    // INITIALIZATION WITH ALL DOTS
    this->_size = n;
    arr = new point[n];
