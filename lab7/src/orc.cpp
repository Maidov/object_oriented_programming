...
Orc::Orc(std::string npcName, const int &x, const int &y) : NPC{npcName, ORC, x, y, RANGE::AR_ORC, RANGE::MR_ORC} {}
...
bool VisitorOrc::visit(std::shared_ptr<NPC> defender, bool backward_compatibility) {
    if ((defender->type() == DRUID) || (!backward_compatibility)){
        defender->kill();
        return true;
    } else {
        return false;
